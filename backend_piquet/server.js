...
app.post('/api/users/register', async (req, res) => {
  try {
    ...
    return res.status(201).json({
      ...
      token,
      user: {
        id: newUser._id.toString(), // ← CORRECTION : Convertir _id en string
        email: newUser.email,
        name: newUser.name,
        role: newUser.role,
      },
...
app.post('/api/users/login', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      token,
      user: {
        id: user._id.toString(), // ← CORRECTION : Convertir _id en string
        email: user.email,
        name: user.name,
        role: user.role,
      },
...
app.listen(PORT, () => {
  ...
  console.log('   GET    /api/users                  - Liste utilisateurs (avec filtre role)');
  console.log('   GET    /api/users/farmers          - Liste fermiers avec données capteurs (ADMIN)');
  console.log('   GET    /api/users/farmers/:id      - Détails fermier spécifique (ADMIN)');
  console.log('   GET    /api/users/admin/stats      - Statistiques dashboard admin (ADMIN)');
